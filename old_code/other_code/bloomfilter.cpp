...
Key::Key(const int32_t key) : key_(key) { }
...
PerfKey::PerfKey(const std::string &key) : key_(key) { }
...
template<typename T>
BloomFilter<T>::BloomFilter(const int32_t n, const double false_positive_p) : bits_(), k_(0), m_(0), n_(0), p_(false_positive_p)
{
...
TestSuite::TestSuite(const int32_t key_nums, const double expected_false_positive_p) : key_nums_(key_nums), expected_p_(expected_false_positive_p), bloom_(key_nums, expected_p_)
{
...
void TestSuite::test_false_positive_rates(bool with_opt) {
    ...
    std::cout << "key_nums_=" << key_nums_ << " expected false positive rate=" << expected_p_ << " real false positive rate" << matches * 1.0 / 10000 << std::endl;
}
