...
Solver::Solver(EventLoop *loop, int connfd):
        loop_(loop),
        ...
        keepAlive_(false)
{
    ...
    channel_->setReadHandler(bind(&Solver::handleRead, this));
    channel_->setWriteHandler(bind(&Solver::handleWrite, this));
    channel_->setConnHandler(bind(&Solver::handleConn, this));
}
...
void Solver::reset()
{
    ...
    if (timer_.lock())
    {
        shared_ptr<mytimer> my_timer(timer_.lock());
        my_timer->clearReq();
        timer_.reset();
    }
}
...
void Solver::handleConn()
{
    ...
    else
    {
        ...
        loop_->RunInLoop(bind(&Solver::handleClose, shared_from_this()));
    }
}


URIState Solver::parseURI()
{
...
HeaderState Solver::parseHeaders()
{
    string &str = inBuffer_;
...
AnalysisState Solver::analysisRequest()
{
    if (method_ == METHOD_POST)
    {
        // ------------------------------------------------------
...
void Solver::handleClose()
{
    connectionState_ = H_DISCONNECTED;
    shared_ptr<Solver> guard(shared_from_this());
    loop_->removeFromPoller(channel_);
}


void Solver::newEvent()
{
